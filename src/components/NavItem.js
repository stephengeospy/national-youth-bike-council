...
function NavItem({ linkTo, children }) {
  return (
    <Link
      to={linkTo}
      className="focus:shadow-outline mt-2 rounded-lg bg-transparent px-4 py-2 text-lg tracking-wide hover:bg-skin-fill-base hover:text-skin-primary focus:bg-skin-fill-accent focus:text-skin-muted focus:outline-none lg:mt-0 lg:ml-4"
    >
      {children}
    </Link>
  );
